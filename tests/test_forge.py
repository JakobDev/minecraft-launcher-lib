...
@pytest.mark.filterwarnings("ignore:.:DeprecationWarning")
def test_list_forge_versions(requests_mock: requests_mock.Mocker) -> None:
    prepare_requests_mock(requests_mock)
...
@pytest.mark.filterwarnings("ignore:.:DeprecationWarning")
def test_find_forge_version(requests_mock: requests_mock.Mocker) -> None:
    prepare_requests_mock(requests_mock)

    assert isinstance(minecraft_launcher_lib.forge.find_forge_version("test1"), str)
    assert minecraft_launcher_lib.forge.find_forge_version("invalid") is None


@pytest.mark.filterwarnings("ignore:.:DeprecationWarning")
def test_is_forge_version_valid(requests_mock: requests_mock.Mocker) -> None:
    prepare_requests_mock(requests_mock)

    assert minecraft_launcher_lib.forge.is_forge_version_valid("test1-1.0") is True
    assert minecraft_launcher_lib.forge.is_forge_version_valid("invalid") is False
